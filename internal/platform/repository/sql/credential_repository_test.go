...
func TestSQLCredentialRepository(t *testing.T) {
	...
	t.Run("Given a credential it should be saved", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Equal(t, credential, res)

		err = cr.UpdateCertificationAnchor(context.Background(), credential.CredentialId, proofsUpdate)
		assert.NoError(t, err)

		res, err = cr.GetCredentialByIssuerAndId(context.Background(), issuer, credentialId)
		assert.NoError(t, err)

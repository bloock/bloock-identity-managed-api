...
func TestSQLCredentialRepository(t *testing.T) {
	...
	require.NoError(t, err)
	var bloockProof json.RawMessage
	err = json.Unmarshal([]byte(blProof), &bloockProof)
	require.NoError(t, err)
	...
	t.Run("Given a credential it should be saved", func(t *testing.T) {
		...
		assert.Equal(t, credential.BloockProof, res.BloockProof)
		assert.Equal(t, "null", string(res.SparseMtProof))

		err = cr.UpdateCertificationAnchor(context.Background(), credential.CredentialId, signatureProof, bloockProof, sparseMtProof)
		assert.NoError(t, err)
